...
def select_sheet() -> object:
    """
    This function helps you choose which sheet to access
    from your Google Spreadsheet.

    It will keep asking you to pick an option until you give a valid choice.
    Here are the options:
    - Press "1" for the "Incomes" sheet
    - Press "2" for the "Expenses" sheet
    - Press "3" for the "Summary" sheet

    Once you make a choice, the function will select the corresponding sheet,
    let you know it's selected,
    ...
    while True:
        choice = input(
            "Please select the sheet you want to access\n"
...
def display_data(data: list):
    ...
    table = tabulate(
        final_data_frame, headers="keys", tablefmt="fancy_grid",
        numalign="center"
    )
    print(table)
...
def modify_or_display_sheet(sheet: object) -> list:
    """
    This function allows you to either show the sheet data
    or add new data to the sheet.
    ...
    if sheet.title != "Summary":
        print(
            "Select an action: view the sheet's data or add new data?\n"
            "Type 'info' to see the data or 'add' to enter new data.\n"
            "If you want to close the current sheet type 'exit'.\n"
        )
    else:
        print(
            "Select an action: Type 'info' to view the sheet's data or\n"
            ...
            if user_choice == "info":
                ...
                if not data[1:]:
                    if sheet.title != "Summary":
                        print(
                            "Sheet is empty!\n"
                            "Type 'add' to add data in this sheet"
                            "or 'exit' to exit the current sheet."
                        )
                    else:
                        print(
                            "Sheet is empty!\n"
                ...
                if sheet.title == "Summary":
                    print(
                        "This sheet does not support manual data addition.\n"
                        "This sheet is read-only."
                        "Type 'info' to see its contents."
                    )
                elif sheet.title == "Incomes":
                    print(
                        "You need to specify: Source, Amount,"
                        "Payment Method, and Description."
                    )
                    print(
                        "Example: Source (Job salary), Amount (4000),"
                        "Payment method (Card, Cash),"
                        "Description (Salary from online job)."
                    )
                    print("")
                    # append current date to data list as first element
                    ...
                    while True:
                        description = input(
                            "write a description for this income\n>>  "
                        ).capitalize()
                    ...
                    data_list.clear()
                    print(
                        "Your inputs has been successfully"
                        "saved in the worksheet."
                    )
                    print("")

                    print("Calculating remaining balance...")
                    print("")
                    # Accessing amount column of the sheet
                    # to return sum of all amounts
                    incomes_amount_column = sheet.col_values(3)
                    incomes_amount_values = incomes_amount_column[1:]
                    sum_of_incomes_amount_values = sum(
                        float(value) for value in incomes_amount_values
                    )
                    # Append total incomes to the list
                    total_incomes_expenses.append(sum_of_incomes_amount_values)
                    ...
                    sum_of_ex_amount_values = sum(
                        float(value) for value in ex_amount_values
                    )
                    ...
                    if not incomes_sheet.get_all_values()[1:]:
                        ...
                        print(
                            "You need to specify: Type, Amount,"
                            "Payment Method, and Description."
                        )
                        print(
                            "Example: Type (Electric bill), Amount (450),"
                            "Payment method (Card, Cash),"
                            "Description (electricity expenses.)."
                        )
                        print("")
                        # append current date to data list asfirst element
                        data_list.append(CURRENT_DATE)
                        # Move every inputs inside while loop
                        # If data is invalid repeat inputs
                        while True:
                        ...
                        while True:
                            description = input(
                                "write a description for this expense\n>>  "
                            ).capitalize()
                            if validate_data_entry(description):
                                data_list.append(description)
                                break
                        ...
                        data_list.clear()
                        print(
                            "Your inputs has been successfully saved"
                            "in the worksheet."
                        )
                        print("")
                        ...
                        sum_of_inc_amount_values = sum(
                            float(value) for value in inc_amount_values
                        )
                        # Append existing total incomes to the list
                        total_incomes_expenses.append(sum_of_inc_amount_values)

                        # Accessing amount column of the sheet
                        # to return sum of all amounts
                        expenses_amount_column = sheet.col_values(3)
                        expenses_amount_values = expenses_amount_column[1:]
                        sum_of_expenses_amount_values = sum(
...
def update_remaining_balance(totals: list):
    """
    Update the remaining balance in the Summary sheet
    based on the total incomes and total expenses.
...
def main():
    ...
    print(
        ...
        "We're here to help you track your expenses and incomes.\n"
